...
layui.use(['form', 'jquery', "layer"], function () {
    ...
    /*if (window.sessionStorage.getItem('userFace') && $(".userAvatar").length > 0) {
        ...
        $("#userFace").attr("src", "public/web/images/face.jpg");
    }*/
    ...
    function lockPage(img) {
        ...
        layer.open({
            ...
            '<div class="admin-header-lock-img"><img src="'+ctx+'public/web/images/' + img+ '" class="userAvatar"/></div>' +
            '<div class="admin-header-lock-name" id="lockUserName">' + $(".adminName")[0].innerText + '</div>' +
            '<div class="input_btn">' +
    ...
    $(".lockcms").on("click", function () {
        $.getJSON(ctx + "user/screen/lock", function (res) {
            if(res.code == 0){
    ...
    $("body").on("click", "#unlock", function () {
        if ($(this).siblings(".admin-header-lock-input").val() == '') {
            ...
            $.post(ctx + "user/screen/unlock", {"password": $(".admin-header-lock-input").val()}, function (result) {
                    if (result === true) {
    ...
    function skins() {
        ...
        $.getJSON(ctx + "user/users/current/skin", function (res) {
            if (res.one && res.one.skin && window.sessionStorage.getItem("skinValue") != res.one.skin) {
                window.sessionStorage.setItem("skinValue", res.one.skin);
                if(res.one.skin == "自定义"){
                    window.sessionStorage.setItem("skin", res.one.customizeSkin);
                    var skinSplit = res.one.customizeSkin.split(',');
                    $(".layui-layout-admin .layui-header").css("background-color", skinSplit[0]);
                    $(".layui-bg-black").css("background-color", skinSplit[1]);
                    $(".hideMenu").css("background-color", skinSplit[2]);
                }else if(res.one.skin == "默认"){
                    $("body").removeClass("orange blue");
                    window.sessionStorage.setItem("skin", "");
                }else if(res.one.skin == "橙色"){
                    $("body").removeClass("blue")
                    $("body").addClass("orange");
                    window.sessionStorage.setItem("skin", "orange");
                }else if(res.one.skin == "蓝色"){
        ...
        layer.open({
            ...
            success: function (index, layero) {
                ...
                form.on("submit(changeSkin)", function (data) {
                    ...
                    $.ajax({
                        type:"post",
                        url: ctx + "user/users/current/skin",
                        data:{skin:data.field.skin, customizeSkin:skinStr},
